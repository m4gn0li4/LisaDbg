...
Options:
    ...
    hmodule, module, m          Displays the modules responsible for the process currently being debugged (only works during debugging)
...
fn print_choice(arg: &[&str]) {
    match arg[0]{
        ...
        _ => eprintln!("{ERR_COLOR}undefined command : {}{RESET_COLOR}", arg[0]),
    }
}
...
pub fn ca_help(help_c: &str) {
    ...
    match help_c {
        ...
        "thread" | "th" => {
            ...
            println!("view thread, view th        : an option of the \"view\" command to see all threads of the process being debugged");
            println!("\x1b[0m");
        }
        "reg" | "register" => {
        ...
        _ => println!("{ERR_COLOR}unknow element : '{}'{RESET_COLOR}", help_c),
    }
    print!("\x1b[0m");
}
...
pub fn help_def(linev: &[&str]) {
    if linev.len() >= 2 {
        match linev[1] {
            ...
            _ => eprintln!("{ERR_COLOR}unknow element '{}'{RESET_COLOR}", linev[1]),
        }
    } else {
        println!("{USAGE_DEF}");
    }
