...
pub unsafe fn parse_header() -> Result<(), io::Error> {
    let mut file = File::open(ALL_ELM.file.clone().unwrap())?;
    let mut dos_header: IMAGE_DOS_HEADER = std::mem::zeroed();
    file.read_exact(slice::from_raw_parts_mut(&mut dos_header as *mut _ as *mut u8, size_of::<IMAGE_DOS_HEADER>()))?;
    ...
    file.read_exact(slice::from_raw_parts_mut(&mut file_header as *mut _ as *mut u8, size_of::<IMAGE_FILE_HEADER>()))?;
    if file_header.Machine == 0x8664 {
        ...
        file.read_exact(slice::from_raw_parts_mut(&mut nt_header_64 as *mut _ as *mut u8, size_of::<IMAGE_NT_HEADERS64>()))?;
        ...
        file.seek(SeekFrom::Start(dos_header.e_lfanew as u64))?;
        file.read_exact(slice::from_raw_parts_mut(&mut nt_header_32 as *mut _ as *mut u8, size_of::<IMAGE_NT_HEADERS64>()))?;
