        ...
        for (i, part) in parts.iter().enumerate() {
            if part.starts_with("--memory=") {
                ...
                flag_type_mem = match mem_type.as_str() {
                    ...
                    _ => {
                        return Err(format!("{ERR_COLOR}Invalid memory type: {mem_type}{RESET_COLOR}"))
                    }
                };
                ...
                if let Some(size_str) = parts.get(i + 1) {
                    match str_to::<usize>(size_str) {
                        ...
                        Err(e) => {
                            return Err(format!("{ERR_COLOR}invalid entry : {e}{RESET_COLOR}"))
                        }
                    }
                } else {
                ...
                match str_to::<i64>(part) {
                    ...
                    Err(_) => {
                        if let Some(sym) = unsafe {
                            symbol::SYMBOLS_V.symbol_file.iter().find(|s| s.name == part.trim_start().trim_end())
                        } {
...
pub fn watchpoint(linev: &[&str]) {
    ...
    unsafe {
        match Watchpts::from_str(&linev[1..].join(" ")) {
            Ok(wt) => {
                if ALL_ELM.watchpts.len() < 4 {
                    ALL_ELM.watchpts.push(wt.clone());
                    println!("{VALID_COLOR}watchpoint was set successfully at offset {}: {}{RESET_COLOR}",wt.format_offset(std::mem::zeroed()), ALL_ELM.watchpts.len())
                } else {
...
pub fn watchpoint_proc(linev: &[&str], ctx: &mut CONTEXT) {
    ...
    match Watchpts::from_str(&linev[1..].join(" ")) {
        Ok(wt) => unsafe {
            if ALL_ELM.watchpts.len() < 4 {
                ALL_ELM.watchpts.push(wt.clone());
                memory::watchpoint::set_dreg(ctx, &wt, ALL_ELM.watchpts.len() - 1);
                println!("{VALID_COLOR}watchpoint {} was set at successfully for watch address {:#x} with access {:?}{RESET_COLOR}", ALL_ELM.watchpts.len(), wt.real_addr64(*ctx), wt.check_type);
            } else {
                eprintln!("{ERR_COLOR}you can only place 4 watchpoints{RESET_COLOR}");
            }
        },
