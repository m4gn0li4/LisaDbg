...
pub fn view_brpkt(linev: &[&str], ctx: CONTEXT, proc_handle: HANDLE) {
    ...
    match elm {
        ...
        "stret" => print_elements(unsafe { &*addr_of!(ALL_ELM.break_ret) }),
        ...
        "break-va" | "bva" => print_elements(unsafe { &*addr_of!(ALL_ELM.break_va) }),
...
fn get_sym_name(func: &RUNTIME_FUNCTION) -> String {
    if let Some(sym) = unsafe {SYMBOLS_V.symbol_file.iter().find(|s|s.offset + BASE_ADDR as i64 == func.BeginAddress as i64 + BASE_ADDR as i64)} {
        format!("<{}>", sym.name)
    }else {
        "".to_string()
    }
