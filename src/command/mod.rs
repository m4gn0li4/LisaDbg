...
pub mod arg {
    ...
    pub fn set_argument(linev: &[&str]) {
        ...
        unsafe { ALL_ELM.arg = Some(linev[1..].join(" ")) }
...
pub mod with_va {
    ...
    pub fn handle_calcule_va(linev: &[&str]) {
        ...
        match str_to::<u64>(linev[1]) {
            ...
            Err(e) => {
                eprintln!("{ERR_COLOR}when transforming rva str '{e}' into u64 : {e}{RESET_COLOR}")
            }
        }
    }
...
pub mod little_secret {
    ...
    pub fn sub_op(linev: &[&str]) {
        ...
        match (str_to::<isize>(linev[1]), str_to::<isize>(linev[2])) {
            ...
            (Err(e), Err(e2)) => {
                eprintln!("{ERR_COLOR}all argument is invalid, 1 : {e} - 2 : {e2}")
            }
        }
        print!("{RESET_COLOR}");
    }
