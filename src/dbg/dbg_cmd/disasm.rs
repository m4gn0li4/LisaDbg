...
use winapi::um::winnt::{CONTEXT, MEMORY_BASIC_INFORMATION, PAGE_EXECUTE_READWRITE, PAGE_READONLY, WOW64_CONTEXT};
...
fn disasm(h_proc: HANDLE, addr: u64, count_str: Option<&&str>) {
    ...
    unsafe {
        if FUNC_INFO.len() != 0 {
            if let Some(func) = FUNC_INFO.iter().find(|f|f.BeginAddress as u64 + BASE_ADDR <= addr && f.EndAddress as u64 + BASE_ADDR > addr) {
            ...
            if VirtualQueryEx(h_proc, addr as LPVOID, &mut mem_info, mem::size_of::<MEMORY_BASIC_INFORMATION>()) == 0 {
    ...
    unsafe {
        ...
        if ReadProcessMemory(h_proc, addr as LPVOID, buffer.as_mut_ptr() as LPVOID, buffer.len(), std::ptr::null_mut()) == 0 {
    ...
    while decoder.can_decode() && i < count {
        ...
        if insn.is_ip_rel_memory_operand() {
            unsafe {
                ...
                let mut mem_info = mem::zeroed();
                if VirtualQueryEx(h_proc, insn.ip_rel_memory_address() as LPVOID, &mut mem_info, mem::size_of::<MEMORY_BASIC_INFORMATION>()) == 0 {
        ...
        println!("{ADDR_COLOR}{:#x}: {BYTES_COLOR}{:<48} {INSTR_COLOR}{insns}{RESET_COLOR}", insn.ip(), byte_str);
    ...
    while decoder.can_decode() && i < count {
        ...
        if insn.near_branch_target() != 0 {
            if let Some(jmp) = result.iter_mut().find(|f|f.addr == insn.near_branch_target()) {
                ...
                result.push(JAddr{addr: insn.near_branch_target(), num_target: 0});
            }
        }
        i += 1
    }
