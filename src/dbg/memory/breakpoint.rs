...
use winapi::um::winnt::{CONTEXT, CONTEXT_ALL, PAGE_EXECUTE_READWRITE, THREAD_ALL_ACCESS, WOW64_CONTEXT, WOW64_CONTEXT_ALL};
use crate::ALL_ELM;
use crate::dbg::{BASE_ADDR, SAVEINSN, SaveInsn, dbg_cmd};
...
pub unsafe fn restore_byte_of_brkpt(h_proc: HANDLE, b_addr: u64) {
    if let Some(insn) = SAVEINSN.iter().find(|s|s.addr == b_addr) {
...
pub unsafe fn handle_br(h_proc: winapi::um::winnt::HANDLE, debug_event: DEBUG_EVENT, breakpoint_addr: u64, continue_dbg: &mut bool) {
    println!("[{DBG_COLOR}Debug{RESET_COLOR}] -> Breakpoint hit at address: {:#x}", breakpoint_addr);
    restore_byte_of_brkpt(h_proc, breakpoint_addr);
    ...
    if ReadProcessMemory(h_proc, breakpoint_addr as LPVOID, b_insn.as_mut_ptr() as LPVOID, 15, &mut 0) == 0 {
        eprintln!("[{ERR_COLOR}Error{RESET_COLOR}] -> Failed to get insn at address {:#x} : {}", breakpoint_addr, io::Error::last_os_error());
        return;
    }
    let mut decoder = Decoder::with_ip(NT_HEADER.unwrap().get_bitness() as u32, &b_insn, breakpoint_addr, DecoderOptions::NONE);
    let mut insn = Instruction::new();
    decoder.decode_out(&mut insn);
    let next_addr = breakpoint_addr + insn.len() as u64;
    let mut last_oc = 0u8;
    ...
    SAVEINSN.push(SaveInsn {addr: next_addr, last_oc});
    ALL_ELM.after_b.push(AfterB {last_addr_b: breakpoint_addr, after_b: next_addr});
...
pub unsafe fn set_breakpoint_in_ret_func(h_proc: HANDLE, debug_event: DEBUG_EVENT, addr: u64) {
    ...
    let rip = match NT_HEADER.unwrap() {
        NtHeaders::Headers32(_) => {
            ...
            if Wow64SetThreadContext(h_thread, &ctx) == 0 {
                eprintln!("[{ERR_COLOR}Error{RESET_COLOR}] -> faile to sub 1 of rip : {}", io::Error::last_os_error());
                return;
            }
            ctx.Eip as u64
        }
