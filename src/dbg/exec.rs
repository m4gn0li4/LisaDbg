        ...
        while continue_dbg {
            ...
            match debug_event.dwDebugEventCode {
                EXCEPTION_DEBUG_EVENT => {
                    ...
                    match debug_event.u.Exception().ExceptionRecord.ExceptionCode {
                        EXCEPTION_BREAKPOINT | STATUS_WX86_BREAKPOINT => {
                            if let Some(hook_func) = ALL_ELM.hook.iter().find(|a| a.target == except_addr - BASE_ADDR) {
                            ...
                            if let Some(after_b) = ALL_ELM.after_b.iter().find(|a|a.after_b == except_addr) {
                                handle_point::handle_after_b(h_proc, *after_b, debug_event);
                            }
                        }
                        EXCEPTION_SINGLE_STEP | STATUS_WX86_SINGLE_STEP => watchpoint::handle_watchpoint(debug_event, h_proc, &mut continue_dbg),
                        ...
                        EXCEPTION_ACCESS_VIOLATION => {
                            ...
                            let query_result = VirtualQueryEx(h_proc, drs as LPVOID, &mut mem_info, mem::size_of::<MEMORY_BASIC_INFORMATION>());
                            if query_result == 0 {
                ...
                CREATE_PROCESS_DEBUG_EVENT => {
                    ...
                    init(h_proc);
                    memory::watchpoint::set_watchpoint(debug_event, h_proc);
                }
